...
/**
 * One scheduling restriction on a lecturer — when, and how densely, it may be given classes.
 ...
 * value's form per type is enforced by lecturer_timetable_constraints_value_check in schema.sql, so a
 ...
 * lecturer's whole constraint set is replaced in one call and can be validated as a whole,
 * the same arrangement {@link LecturerWorkloadConstraint} uses.
 */
@Data
@GraphQLEntity(table = "lecturer_timetable_constraints")
@PermissionParent(value = Lecturer.class, joinColumn = "lecturer_id")
public class LecturerTimetableConstraint {

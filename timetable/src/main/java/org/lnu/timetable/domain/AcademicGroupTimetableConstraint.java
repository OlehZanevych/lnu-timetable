...
/**
 * One scheduling restriction on an academic group — when, and how densely, it may be given classes.
 ...
 * value's form per type is enforced by academic_group_timetable_constraints_value_check in schema.sql, so a
 * malformed string is rejected by the database rather than silently mis-read.
 * <p>
 * Rows are written exclusively through AcademicGroup's create/update mutations via the
 * {@code timetableConstraints} nested list — there are no standalone mutations, so a
 * group's whole constraint set is replaced in one call and can be validated as a whole,
 * the same arrangement {@link LecturerWorkloadConstraint} uses.
 */
@Data
@GraphQLEntity(table = "academic_group_timetable_constraints")
@PermissionParent(value = AcademicGroup.class, joinColumn = "academic_group_id")
public class AcademicGroupTimetableConstraint {
